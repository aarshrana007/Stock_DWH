...
from .utils.io import write_parquet, read_parquet, write_json, list_parquet_files
...
def wh_join(base, *parts):
    """Join warehouse paths for both local Path and S3 string."""
...
def _write_partitioned(
    ...
    if partition_col not in df.columns:
        p = wh_join(root, filename)
        write_parquet(df, p)
        return
    for dt, chunk in df.groupby(partition_col):
        p = wh_join(root, f"{partition_col}={dt}", filename)
...
def build_silver() -> None:
    ...
    news_root = wh_join(paths.bronze, "raw_news")
    px_root = wh_join(paths.bronze, "raw_prices")
...
def infer_run() -> None:
    ...
    _write_partitioned(mart, wh_join(paths.gold, "mart_market_snapshot_topbottom"), filename="snapshot.parquet")
